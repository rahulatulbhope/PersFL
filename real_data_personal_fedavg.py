...
def accuracy_dataset_per_label(model, dataset,no_labels):
    ...
    return np.average(np.nan_to_num(cm_matrix_1))
    ...
    for try_1 in range(0,1):
        ...
        for i in range(n_iter):
            ...
            if i%10==0:
                ...
                for party in list(np.arange(0,num_clients)):
                    model_p = deepcopy(model)

                    layer_to_freeze = model_p.conv1
                    # Freeze the parameters in the chosen layer
                    for param in layer_to_freeze.parameters():
                        param.requires_grad = False

                    layer_to_freeze = model_p.conv2
                    # Freeze the parameters in the chosen layer
                    for param in layer_to_freeze.parameters():
                        param.requires_grad = False

                    layer_to_freeze = model_p.conv3
                    # Freeze the parameters in the chosen layer
                    for param in layer_to_freeze.parameters():
                        param.requires_grad = False

                    layer_to_freeze = model_p.conv4
                    # Freeze the parameters in the chosen layer
                    for param in layer_to_freeze.parameters():
                        param.requires_grad = False


                    local_loss=local_learning(model_p,mu,local_optimizer,
                    training_sets[party],epochs,loss_f)

                    acc_hist.append(accuracy_dataset_per_label(model, testing_sets[party],5))
                    ...
                    #loss_hist+=[[float(loss_dataset(model, training_sets[dl], loss_f).detach()) 
                    #    for dl in training_sets]]
                    #acc_hist=[[accuracy_dataset_per_label(model, testing_sets[dl],5) for dl in testing_sets]]
                ...
                server_acc = np.average(acc_hist)

                print(f'====> i: {i+1} Loss: {server_loss} Server Test Accuracy: {server_acc}')
                
        
                with open('cs_284/Real_data_'+str(how_many_parts)+'_dir_'+str(int(alpha*10))+'_redo_non_clus_poison_arrythmia_lenet'+str(int(alpha*100))+str(top)+'fedavg.csv', 'a') as f:
                    # create the csv writer
                    writer = csv.writer(f)
        
                    # write a row to the csv file
                    writer.writerow([str(i),str(server_loss),str(server_acc)])#str(i)+"," + str(server_loss)+","+str(server_acc)+","+str(mu))
                    #writer.write("\n")
...
with open('mit_data_test_43.pickle', 'rb') as handle:
    X_dict_test= pickle.load(handle)

with open('mit_data_labels_test_43.pickle', 'rb') as handle:
